...
export function ContactFullScreen({ isOpen, onClose }: MenuModalProps) {
  const gaEventTracker = useGoogleAnalyticsEventTracker("Contact Desktop");
  ...
  const postSubmission = async () => {
    ...
    try {
      ...
      console.log(result);
      gaEventTracker("Contact Submit", "Contact Submit");
      ReactGA.event({
        category: "Contact Desktop",
        action: "Contact Submit",
        label: "Contact Submit in Desktop",
      });
      contactUsDesktop();
      setMessage({
        class: "bg-green-500",
